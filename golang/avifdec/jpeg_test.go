...
func TestJPEGOutput(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Failed to decode AVIF to JPEG: %v", err)
	}
	...
	t.Logf("Successfully decoded AVIF (%d bytes) to JPEG (%d bytes, quality=%d)",
		len(avifData), len(jpegData), opts.JPEGQuality)
}
...
func TestJPEGQuality(t *testing.T) {
	...
	for _, quality := range qualities {
		...
		if err != nil {
			t.Fatalf("Failed to decode AVIF to JPEG (quality=%d): %v", quality, err)
		}
...
func TestPNGvsJPEGOutput(t *testing.T) {
	...
	pngData, err := pngCmd.Run(avifData)
	if err != nil {
	...
	jpegData, err := jpegCmd.Run(avifData)
	if err != nil {
